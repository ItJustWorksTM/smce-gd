...
static func instance_list(boards: TrackedArrayBase, active_sketch: Tracked) -> Callable: return func(c: Ctx):
    ...
    var minimized := Track.value(false)
    ...
    c.child(func(c: Ctx):
        ...
        c.with("visible", Track.map(minimized, func(v): return !v))
        c.child(func(c: Ctx):
            c.inherits(VBoxContainer)
            c.child(func(c: Ctx):
            ...
            c.child_opt(Ui.map_children(boards, func(i, __): return func(c: Ctx):
                var inverse = Track.combine_map([boards as Tracked, i as Tracked], func(a,c):
                    return a.size() - c - 1 
                )
                
                var should_be_pressed = Track.dedup(Track.combine_map([boards as Tracked, active_sketch as Tracked, i as Tracked], func(a,b,c):
                    if b < 0: return false
                ...
                c.inherits(Widgets.button())
                c.with("text", Track.map(inverse, func(i): return str(i +1)))
