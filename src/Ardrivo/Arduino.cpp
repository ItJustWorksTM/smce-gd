...
void pinMode(int pin, bool mode) {
    auto error = [=](const char* msg) { std::cerr << "ERROR: pinMode(" << pin << ", " << (mode ? "OUTPUT" : "INPUT") << "): " << msg << std::endl; };
    maybe_init();
...
int digitalRead(int pin) {
    auto error = [=](const char* msg) { return std::cerr << "ERROR: digitalRead(" << pin << "): " << msg << std::endl, 0; };
    maybe_init();
    auto vpin = board_view.pins[pin];
...
void digitalWrite(int pin, bool value) {
    auto error = [=](const char* msg) { std::cerr << "ERROR: digitalWrite(" << pin << ", " << (value ? "HIGH" : "LOW") << "): " << msg << std::endl; };
    maybe_init();
    auto vpin = board_view.pins[pin];
...
int analogRead(int pin) {
    auto error = [=](const char* msg) { return std::cerr << "ERROR: analogRead(" << pin << "): " << msg << std::endl, 0; };
    maybe_init();
    auto vpin = board_view.pins[pin];
...
void analogWrite(int pin, byte value) {
    auto error = [=](const char* msg) { std::cerr << "ERROR: analogWrite(" << pin << ", " << static_cast<int>(value) << "): " << msg << std::endl; };
    maybe_init();
    auto vpin = board_view.pins[pin];
